...
void	copyMatrix(Matrix _Aout, Matrix _Bin)
{
	if (_Aout.rows != _Bin.rows || _Aout.cols != _Bin.cols) {
		printf("\n**************************************************");
	...
	for (int i = 0; i < _Bin.rows; i++)
		for (int j = 0; j < _Bin.cols; j++)
			_Aout.at[i][j] = _Bin.at[i][j];
}

/*
// Up to 2025. Don't Use it for v2026~
// Copy matrix Elements from A to B
